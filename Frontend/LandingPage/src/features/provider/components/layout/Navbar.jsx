...
const Navbar = () => {
  ...
  const isCurrentPath = (path) => {
    ...
    if (path === `${providerBasePath}/`) {
      return location.pathname === providerBasePath || location.pathname === `${providerBasePath}/`;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link to={providerBasePath} className="flex-shrink-0 flex items-center">
          ...
          <div className="hidden md:flex md:items-center">
            ...
            <div className="relative ml-3">
              <div>
                ...
                >
                  <span className="font-medium text-sm hidden sm:block">{user?.name || 'Provider'}</span>
              ...
              {isProfileOpen && (
                <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 z-50">
                  <Link to={`${providerBasePath}/profile`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                    Your Profile
                  </Link>
                  <Link to={`${providerBasePath}/settings`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                    Settings
                  </Link>
        ...
        <div className="md:hidden">
          ...
          <div className="pt-4 pb-3 border-t border-gray-200">
            <div className="flex items-center px-4">
              ...
              <div className="ml-3">
                <div className="text-base font-medium text-gray-800">{user?.name || 'Provider Name'}</div>
                <div className="text-sm font-medium text-gray-500">{user?.email || 'provider@example.com'}</div>
              </div>
              {/*
